...
public class PacketSpawnTrailMixPig extends AbstractPacket
{
    ...
    {
        if(livingEntId == Minecraft.getMinecraft().player.getEntityId())
        {
            Minecraft.getMinecraft().player.renderArmPitch += 100F;
        }
        Entity ent1 = Minecraft.getMinecraft().world.getEntityByID(livingEntId);
        if(ent1 instanceof EntityZombie)
        {
            ((EntityZombie)ent1).renderYawOffset = ent1.rotationYaw = livingRotYaw;
        }
        Entity ent = Minecraft.getMinecraft().world.getEntityByID(pigEntId);
        if(ent != null && ent instanceof EntityPig)
        {
            ...
            if(!hasTrailMix)
            {
                for (int var1 = 0; var1 < 20; ++var1)
                {
                    ...
                    double var8 = 1.0D;
                    pig.world.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, pig.posX + (double)(pig.getRNG().nextFloat() * pig.width * 2.0F) - (double)pig.width - var2 * var8, pig.posY + (double)(pig.getRNG().nextFloat() * pig.height) - var4 * var8, pig.posZ + (double)(pig.getRNG().nextFloat() * pig.width * 2.0F) - (double)pig.width - var6 * var8, var2, var4, var6);
                }
            }
            else
            {
